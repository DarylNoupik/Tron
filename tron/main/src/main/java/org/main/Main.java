...
import org.model.TronMapStory;
import org.model.TronMapSurvival;
import org.model.TronMapTwoPlayer;
import org.view.Picture;
...
public class Main implements Runnable{
	// true if the instructions page is in the frame
		...
		public void run() {
			 ...
			 * actionPerformed(ActionEvent e) { frame.remove(mainMenu); frame.add(playMenu);
			 * frame.update(frame.getGraphics()); playMenu.revalidate(); } });
			 */

			/*
			 * instructions.addActionListener(new ActionListener() { public void
			 * actionPerformed(ActionEvent e) { if (instructOn) {
			 * mainMenu.remove(instrPict); mainMenu.add(pict); instructions.setIcon(new
			 * ImageIcon("instructions_before.png")); } else if (!instructOn) {
			 * mainMenu.remove(pict); mainMenu.add(instrPict); instructions.setIcon(new
			 * ImageIcon("main_menu.png")); } mainMenu.revalidate(); frame.repaint();
			 * instructOn = !instructOn; } });
			 * 
			 * quit.addActionListener(new ActionListener() { public void
			 * actionPerformed(ActionEvent e) { System.exit(1); } });
			 * 
			 * highScores.addActionListener(new ActionListener() { public void
			 * actionPerformed(ActionEvent e) { if (scoresOn) { playMenuUpper.removeAll();
			 * playMenuUpper.add(modes); playMenuUpper.add(menuPict); } else if (!scoresOn)
			 * { playMenuUpper.remove(menuPict); playMenuUpper.add(levelSurv.getHighs()); }
			 * scoresOn = !scoresOn; playMenuUpper.revalidate(); frame.repaint(); } });
			 * 
			 * back.addActionListener(new ActionListener() { public void
			 * actionPerformed(ActionEvent e) { frame.remove(playMenu); frame.add(mainMenu);
			 * frame.update(frame.getGraphics()); } });
			 * 
			 * survival.addActionListener(new ActionListener() { public void
			 * actionPerformed(ActionEvent e) { frame.remove(playMenu); frame.setLayout(new
			 * BorderLayout()); frame.add(levelSurv, BorderLayout.CENTER);
			 * frame.add(survMenu, BorderLayout.SOUTH); frame.update(frame.getGraphics());
			 * levelSurv.requestFocusInWindow(); levelSurv.revalidate(); levelSurv.reset();
			 * } });
			 * 
			 * reset.addActionListener(new ActionListener() { public void
			 * actionPerformed(ActionEvent e) { levelSurv.reset(); } });
			 * 
			 * exit.addActionListener(new ActionListener() { public void
			 * actionPerformed(ActionEvent e) { frame.remove(levelSurv);
			 * frame.remove(survMenu); frame.add(playMenu);
			 * frame.update(frame.getGraphics()); playMenu.revalidate(); } });
			 */
			...
			/*
			 * resetTwo.addActionListener(new ActionListener() { public void
			 * actionPerformed(ActionEvent e) { levelTwoPlayer.reset(); } });
			 * 
			 * exitTwo.addActionListener(new ActionListener() { public void
			 * actionPerformed(ActionEvent e) { frame.remove(levelTwoPlayer);
			 * frame.remove(twoMenu); frame.add(playMenu);
			 * frame.update(frame.getGraphics()); playMenu.revalidate();
			 * levelTwoPlayer.restartGame(); } });
			 */
			// story.addActionListener(new ActionListener() { public void
			// actionPerformed(ActionEvent e) { frame.remove(playMenu); frame.setLayout(new
			// BorderLayout()); frame.add(levelStory, BorderLayout.CENTER);
			// frame.add(storyMenu, BorderLayout.SOUTH); frame.update(frame.getGraphics());
			...
			exitStory.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) { // frame.setLayout(null);
